...
from modelss.ade20k import SegmentationModule, NUM_CLASS, segm_options
...
class PairwiseScore(EvaluatorScore, ABC):
    ...
    def get_value(self, groups=None, states=None):
        ...
        """
        individual_values = torch.stack(states, dim=-1).reshape(-1).cpu().numpy() if states is not None \
