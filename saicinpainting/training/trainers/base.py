...
class BaseInpaintingTrainingModule(ptl.LightningModule):
    def __init__(self, config, use_ddp, *args,  predict_only=False, visualize_each_iters=100,
        ...
        self.generator_rev = make_generator(config, **self.config.generator)
    ...
    def configure_optimizers(self):
        ...
        optimizerg = make_optimizer(self.generator.parameters(), **self.config.optimizers.generator)
        optimizerd = make_optimizer(discriminator_params, **self.config.optimizers.discriminator)
        schedulerg = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizerg, T_0=1300, T_mult=1)
        schedulerd = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizerd, T_0=1300, T_mult=1)
